...
## 24. 摄像头黑屏且状态残留 — 根因是程序强制手动曝光（非供电问题）✅ 已解决

| 属性 | 值 |
|------|-----|
| **模块** | `src/main.cpp`（V4L2 初始化曝光控制段） |
| **现象** | 浏览器 MJPEG 画面全黑；v4l2-ctl 抓帧只有 ~6.7KB（正常应 ~100KB）；程序退出后 v4l2-ctl 抓帧仍黑 |
| **严重程度** | ❌ 严重 — 画面黑屏，且污染摄像头状态 |
| **排查耗时** | 极长（误判供电 → 最终定位程序） |

### 排查过程（重要教训：一度误判为供电问题）

1. **误判 USB 供电**：换 USB 口后 v4l2-ctl 抓帧大小变化（6.7KB ↔ 100KB），曾以为供电不足。**实际不是供电问题**——同一"正常口"上 v4l2-ctl 的帧大小也在波动，且摄像头插电脑一直正常。
2. **关键证据（程序污染摄像头）**：
   - 运行 smartcam **之前**：`v4l2-ctl --stream-count=3` 抓帧 = **~317KB（~100KB/帧，正常画面）**
   - 运行 smartcam **之后**：同命令抓帧 = **~20KB（~6.7KB/帧，全黑）**
   - **程序运行后把摄像头固件搞进了"黑帧"状态，且状态残留**（V4L2 设备 close 后不重置，只能插拔 USB 重置固件）。
3. **用 v4l2-ctl 做基准定位**：v4l2-ctl 是最原始工具，它抓到的帧也黑 → 排除"程序取帧/解码/推流"环节，指向"程序对摄像头做了什么操作污染了固件"。
4. **版本对比二分**：`954e19a`（无曝光功能）→ 浏览器正常 + 不污染；`70fa0aa`（强制曝光 300）→ 黑屏 + 污染。**污染源锁定在曝光控制**。
5. **v4l2-ctl 曝光实验**：设 `exposure_absolute=300` 后抓帧变小（黑），不设则正常 → **强制手动曝光 300 触发该摄像头固件进入黑帧模式，且状态残留**。

### 解决

去掉初始化时的强制手动曝光设置（`V4L2_CID_EXPOSURE_AUTO=1` + `V4L2_CID_EXPOSURE_ABSOLUTE=300`），**保持摄像头自动曝光**，曝光值仅查询供 GUI 滑块显示：

```cpp
// 只查询、不写硬件（保留自动曝光）
capture->getControl(V4L2_CID_EXPOSURE_AUTO, expVal);
gui.setExposureRange(absMin, absMax, absStep, def);   // 仅显示范围
// 不再 setControl(V4L2_CID_EXPOSURE_AUTO, 1)
// 不再 setControl(V4L2_CID_EXPOSURE_ABSOLUTE, 300)
```

修复后：浏览器正常画面，且**退出程序后摄像头状态不再被污染**（v4l2-ctl 仍能抓大帧，无需插拔重置）。

### 经验教训（重要）

| 教训 | 说明 |
|------|------|
| **帧大小是黑帧的快速指标** | 640x480 MJPEG：~100KB = 正常画面；~6.7KB ≈ 全黑/纯色。下载帧打开确认更直观 |
| **v4l2-ctl 是最佳基准** | 它抓的帧黑 = 摄像头问题/程序污染，不是程序取帧/推流环节的问题 |
| **运行程序前后对比 v4l2-ctl** | 若运行前正常、运行后变黑 → 程序污染摄像头固件（状态残留）|
| **排查时必须插拔 USB 重置** | 某些 UVC 固件状态在 V4L2 close 后不重置，需 USB 重新枚举才能恢复 |
...
> **说明（2026-08-03 更新）**：上方为历史崩溃日志快照（当时用 `<dev-ip>` 占位符）。现版本 `main.cpp` 已新增 `getLocalIPv4()`，启动时会自动查询本机 IPv4 并打印实际访问地址（如 `http://192.168.1.100:8080/`），不再显示占位符。
