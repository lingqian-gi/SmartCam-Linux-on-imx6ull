...
deploy_to_board() {
    ...
    echo "  # ⚠️ 必须先设置触摸输入环境变量，否则按钮无法点击!"
    echo "  export QT_QPA_PLATFORM=linuxfb:fb=/dev/fb0"
    echo "  export QT_QPA_FB_HIDECURSOR=1"
    echo "  export QT_QPA_EVDEV_TOUCHSCREEN_PARAMETERS=/dev/input/event1:rotate=0"
    echo "  # 如不生效，尝试加载 evdevtouch 插件:"
    echo "  # export QT_QPA_GENERIC_PLUGINS=evdevtouch"
    echo ""
    echo "  smartcam --device /dev/video0 --http-port 8080 --fmt mjpeg"
    echo ""
    echo "  # 排查触摸设备:"
    echo "  ls /dev/input/event* && cat /dev/input/event1 | hexdump"
