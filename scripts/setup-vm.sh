...
deploy_to_board() {
    ...
    echo "  # linuxfb 自动检测触摸设备，无需额外环境变量"
    echo "  export QT_QPA_PLATFORM=linuxfb:fb=/dev/fb0"
    echo "  export QT_QPA_FB_HIDECURSOR=1"
    echo ""
    echo "  # 如果触摸无响应，修复设备权限:"
    echo "  sudo chmod 666 /dev/input/event2       # 临时（重启失效）"
    echo "  sudo usermod -a -G input \$USER         # 永久（重新登录生效）"
    echo ""
    echo "  smartcam --device /dev/video0 --http-port 8080 --fmt mjpeg"
    echo ""
    echo "  # 排查触摸设备:"
    echo "  ls -la /dev/input/event* && cat /dev/input/event2 | hexdump"
